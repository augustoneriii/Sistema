...
function AgendaProfissional() {
    ...
    useEffect(() => {
        ...
        AgendaProfissionalService.getProfissionais(currentToken)
            .then(response => {
                setProfissionais(response.data); // Acesso � array de profissionais
            })
            .catch(error => {
    ...
    const saveAgenda = () => {
        ...
        if (agenda.dia && agenda.hora && agenda.diaSemana && agenda.profissionalId != null) {
            ...
            let postAgenda = {
                dia: agenda.dia, // Corrigido para letra min�scula
                hora: agenda.hora, // Corrigido para letra min�scula
                profissionalId: agenda.profissionalId,
                diaSemana: agenda.diaSemana, // Corrigido para letra min�scula
            ...
            toast.current.show({ severity: 'error', summary: 'Erro', detail: 'Todos os campos s�o obrigat�rios.', life: 3000 });
        }
    };
    ...
    const editAgenda = (agenda) => {
        ...
        setAgendaDialog(true); // Abre o di�logo de edi��o
    };


    const onProfissionalChange = (e) => {
        const selectedProfissionalId = e.value; // Obt�m o ID do profissional selecionado
    ...
    const onInputChange = (e, campo) => {
        ...
        if (campo === 'dia' || campo === 'hora') {
            // Converta para string apenas se n�o for null
    ...
    const formatDate = (date) => {
        ...
        const month = (date.getMonth() + 1).toString().padStart(2, '0'); // Adiciona um zero � esquerda se necess�rio
        const day = date.getDate().toString().padStart(2, '0'); // Adiciona um zero � esquerda se necess�rio
    ...
    /*const deletePacienteDialogFooter = (
        <React.Fragment>
            <Button label="N�o" icon="pi pi-times" className="border-round p-button-text" onClick={hideDeletePacienteDialog} />
        ...
        <Modal modal={false} header={headerTela} visible={agendaProfissionalVisible} style={{ width: '70vw' }} onHide={() => setAgendaProfissionalVisible(false)}>
            ...
            <Dialog visible={agendaDialog} style={{ width: '850px', margin: 'auto' }} header="Detalhes da Agenda" modal className="p-fluid" footer={agendaDialogFooter} onHide={hideDialog}>
                ...
                <div className="field ">
                    ...
                    {submitted && !agenda.dia && <small className="p-error">Dia � obrigat�rio.</small>}
                </div>
